...
public class ReservationService {
    ...
    /**
     * Creates a temporary reservation for the Saga. The room remains
     * unavailable while payment is being processed; a failed Saga compensates
     * this state by cancelling the reservation.
     */
    @Transactional
    public ReservationResponse reservePendingPayment(ReservationRequest request) {
        return createReservation(request, ReservationStatus.PENDING_PAYMENT);
    }
    ...
    public ReservationResponse confirm(UUID reservationId) {
        ...
        if (reservation.getStatus() != ReservationStatus.PENDING_PAYMENT) {
            throw new IllegalStateException("Reservation " + reservationId
                    + " cannot be confirmed from status " + reservation.getStatus());
        }
    ...
    private ReservationResponse createReservation(ReservationRequest request, ReservationStatus status) {
